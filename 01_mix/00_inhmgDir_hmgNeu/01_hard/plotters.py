...
def plot_gradient_backward(
    ...
    for l in range(model.depth):
        ...
        _grads_glb.append(tf.reshape(grad_glb, [-1]))
        _grads_pde.append(tf.reshape(grad_pde, [-1]))
        # _grads_bc.append(tf.reshape(grad_bc, [-1]))
    ...
    plt.subplot(1, 2, 1)
    for l in range(model.depth):
        # sns.histplot(_grads_glb[l], stat="count", element="step", fill=False, alpha=.7, label=r"layer-"+str(l))
        sns.kdeplot(_grads_glb[l], fill=False, alpha=.7, label=r"layer-"+str(l))
    ...
    plt.subplot(1, 2, 2)
    for l in range(model.depth):
        # sns.histplot(_grads_pde[l], stat="count", element="step", fill=False, alpha=.7, label=r"layer-"+str(l))
        sns.kdeplot(_grads_pde[l], fill=False, alpha=.7, label=r"layer-"+str(l))
    plt.legend(loc="upper right")
    plt.xlabel("Gradient")
    plt.ylabel("Density")
    ...
    plt.tight_layout()
    file_name = "grad_backward_" + file_name + "_" + str(epoch)
    for file_type in file_types:
        plt.savefig(os.path.join(save_path, file_name + file_type), dpi=300)
    plt.close()
