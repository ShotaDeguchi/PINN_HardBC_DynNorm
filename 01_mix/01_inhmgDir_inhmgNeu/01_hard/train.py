...
def main(args):
    ...
    x_reshape2 = X_ref.reshape(-1, 1)
    y_reshape2 = Y_ref.reshape(-1, 1)
    u_reshape2 = u_ref.reshape(-1, 1)
    ...
    plt.tight_layout()
    plt.savefig(path_fig / "poisson_reference.svg")
    plt.close()
    ...
    # pde residual
    x_pde = tf.random.uniform((N_pde, 1), xmin, xmax, dtype=tf.float64, seed=42)
    y_pde = tf.random.uniform((N_pde, 1), ymin, ymax, dtype=tf.float64, seed=42)
    # north
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(path_fig / "collocation_points.svg")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(path_fig / "feature_scaling.svg")
    plt.close()
    ...
    for epoch in range(0, args.epochs+1):
        ...
        # u_, u_x_, u_y_, u_xx_, u_yy_, phi, phi_Dir, phi_Neu, phi_Neu_x, phi_Neu_y, r_ = model.infer(x_reshape, y_reshape)
        # # u_err = u_ - u_reshape
        # print(f'x = {tf.squeeze(x_reshape)}')
        # print(f'y = {tf.squeeze(y_reshape)}')

        x_reshape2 = tf.cast(x_reshape2, dtype=tf.float64)
        y_reshape2 = tf.cast(y_reshape2, dtype=tf.float64)
        u_, u_x_, u_y_, u_xx_, u_yy_, phi, phi_Dir, phi_Neu, phi_Neu_x, phi_Neu_y, r_ = model.infer(x_reshape2, y_reshape2)
        u_err = u_ - u_reshape2
        print(f'x = {tf.squeeze(x_reshape2)}')
        print(f'y = {tf.squeeze(y_reshape2)}')
        print(f'u_ = {tf.squeeze(u_)}')
        print(f'u_x_ = {tf.squeeze(u_x_)}')
        print(f'u_y_ = {tf.squeeze(u_y_)}')
        print(f'u_xx_ = {tf.squeeze(u_xx_)}')
        print(f'u_yy_ = {tf.squeeze(u_yy_)}')
        print(f'phi = {tf.squeeze(phi)}')
        print(f'phi_Dir = {tf.squeeze(phi_Dir)}')
        print(f'phi_Neu = {tf.squeeze(phi_Neu)}')
        print(f'phi_Neu_x = {tf.squeeze(phi_Neu_x)}')
        print(f'phi_Neu_y = {tf.squeeze(phi_Neu_y)}')
        r_err = r_
        ...
        if epoch % 100 == 0:
            plot_comparison(
                x=x_reshape2, y=y_reshape2, u_ref=u_reshape2, u_inf=u_, u_err=u_err,
                umin=0., umax=1., vmin=0., vmax=5e-2,
            ...
            plot_comparison(
                x=x_reshape2, y=y_reshape2, u_ref=u_x_, u_inf=u_y_, u_err=u_err,
                umin=-1., umax=1., vmin=0., vmax=5e-2,
                xmin=0., xmax=1., xlabel=r"$x$",
                ymin=0., ymax=1., ylabel=r"$y$",
                epoch=epoch,
                save_path=path_fig,
            ...
            plot_3images(
                ...
                u1=u_, u1min=0., u1max=1.,   u1label=r"$\tilde{u}$", u1cmap="turbo",
                u2=np.abs(u_err), u2min=0., u2max=5e-2, u2label=r"$\left| \tilde{u} - u \right|$", u2cmap="turbo",
                u3=u_y_,  u3min=-1., u3max=1., u3label=r"$\partial_y \tilde{u}$", u3cmap="seismic",
                epoch=epoch,
                save_path=path_fig,
            ...
            plot_4images(
                ...
                u2=u_, u2min=0., u2max=1.,   u2label=r"$\tilde{u}$", u2cmap="turbo",
                u3=np.abs(u_err), u3min=0., u3max=5e-2, u3label=r"$\left| \tilde{u} - u \right|$", u3cmap="turbo",
                u4=-u_y_,  u4min=center-margin, u4max=center+margin, u4label=r"$-\partial_y \tilde{u}$", u4cmap="seismic",
                epoch=epoch,
                save_path=path_fig,
